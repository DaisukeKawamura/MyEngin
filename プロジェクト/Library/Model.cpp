...
void Model3D::LoadObjModel(const char *modelName)
{
	...
	//頂点バッファ生成
	result = Object3DManager::GetDev()->CreateCommittedResource(
		&HEAP_PROP,
...
void Model3D::CreatePlaneModelXY(float x_size, float y_size, float uv_x, float uv_y, UINT useTexNum)
{
	//頂点データ作成
	Vertex plane[4] = {
...
void Model3D::CreatePlaneModelXZ(float x_size, float z_size, float uv_x, float uv_y, UINT useTexNum)
{
	//頂点データ作成
	Vertex plane[4] = {
...
void Model3D::CreatePlaneModelYZ(float y_size, float z_size, float uv_x, float uv_y, UINT useTexNum)
{
	//頂点データ作成
	Vertex plane[4] = {
...
void Model3D::ChangeTexAnimationNumber(int useAnimNum)
{
	//範囲外参照を検知
