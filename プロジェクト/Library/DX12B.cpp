...
bool DX12B::CreateRenderTargetView()
{
	HRESULT result = S_FALSE;
...
void DX12B::Initialize(WinAPI *win)
{
	// nullptrチェック
	assert(win);
...
void DX12B::StartDraw()
{
	...
	//上と同じエラーに対処
	auto viewport_temp = CD3DX12_VIEWPORT(0.0f, 0.0f, WinAPI::window_width, WinAPI::window_height);
	auto rect_temp = CD3DX12_RECT(0, 0, WinAPI::window_width, WinAPI::window_height);
	// ビューポートの設定
...
bool DX12B::InitInput(WinAPI *win)
{
	//キー入力初期化
	return Input::Get()->Init(win->GetWNDCLASSEX(), win->GetHWND());
}

bool DX12B::CreateDsvHeapForIngui()
{
	//初期化はimgui管理部が実行
	return true;
}
