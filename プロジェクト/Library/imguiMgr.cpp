...
bool ImguiMgr::InitImgui(ID3D12Device *dev, HWND hwnd)
{
	...
	//windows用初期化
	bool bin_result = ImGui_ImplWin32_Init(WinAPI::GetHWND());
	if (!bin_result) {
		return false;
	}
...
void ImguiMgr::EndDrawImgui()
{
	...
	//デスクリプタヒープをセット
	DX12B::Get()->GetGCommandList()->SetDescriptorHeaps(
		1, dsvHeapForImgui.GetAddressOf()
	);

	//描画実行
	ImGui_ImplDX12_RenderDrawData(ImGui::GetDrawData(), DX12B::Get()->GetGCommandList());
}
