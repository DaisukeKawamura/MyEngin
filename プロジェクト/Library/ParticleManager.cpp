...
void ParticleManager::Draw(UINT drawTexNum)
{
	...
	cmd->SetGraphicsRootDescriptorTable(1,
		CD3DX12_GPU_DESCRIPTOR_HANDLE(TexManager::texDsvHeap.Get()->GetGPUDescriptorHandleForHeapStart(),
			drawTexNum, DX12B::Get()->GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV)));

	// 描画コマンド
	cmd->DrawInstanced(drawNum, 1, 0, 0);
}

void ParticleManager::Add(int life, Vector3 pos, Vector3 vel, Vector3 acc, 
...
void ParticleManager::InitializeGraphicsPipeline() {
	...
	// グラフィックスパイプラインの生成
	result = DX12B::Get()->GetDevice()->CreateGraphicsPipelineState(&gpipeline, IID_PPV_ARGS(&pipeline));
