...
ID3D12Device *Object3DManager::dev = nullptr;

bool Object3DManager::CreateObject3DManager(ID3D12Device *dev, int window_w, int window_h)
{
    //ウィンドウサイズ設定
...
HRESULT Object3DManager::QuickDrawShadersCompile()
{
    HRESULT result;     //リザルト
...
Pipeline3D Object3DManager::Create3DPipelineState(ID3D12Device *dev)
{
    HRESULT result;
...
void Object3DManager::LoadObject3DTexture(UINT &texNumber, string filename, ID3D12Device *dev)
{
...
Object3d *Object3DManager::CreateObject3d(Model3D *modelData)
{
...
void Object3DManager::DeleteObject3d(Object3d *obj)
{
...
void Object3DManager::UpdateAllObjects()
{
    //すべてのオブジェクトを更新する
    for (int i = 0; i < objects.size(); i++) {
...
void Object3DManager::SetCommonBeginDrawObject3D()
{
    //パイプラインステートをセット
    DX12B::Get()->GetGCommandList()->SetPipelineState(object3dPipelineSet.pipelinestate.Get());
    //ルートシグネチャをセット
    DX12B::Get()->GetGCommandList()->SetGraphicsRootSignature(object3dPipelineSet.rootsignature.Get());
    //プリミティブ形状設定
    DX12B::Get()->GetGCommandList()->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    //デスクリプタヒープ設定
    ID3D12DescriptorHeap *ppHeaps[] = { TexManager::texDsvHeap.Get() };
    DX12B::Get()->GetGCommandList()->SetDescriptorHeaps(_countof(ppHeaps), ppHeaps);
}

Object3d *CreateObject3d(Model3D *modelData, Vector3 pos)
{
    //返却用のポインタ変数にObject3Dを作成
    Object3d *result = Object3DManager::Get()->CreateObject3d(modelData);
...
void DrawObject3d(Object3d *obj)
{
    ...
    //オブジェクト描画
    obj->DrawModel3D(DX12B::Get()->GetGCommandList(), DX12B::Get()->GetDevice());
}
